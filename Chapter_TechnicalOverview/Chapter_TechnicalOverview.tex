...
In definition~\ref{def:cqentropy}, $P_{\text{guess}}(X|B)$ has the operational meaning of being the probability of guessing $x$ having access to system $B$. Next, we present Lemma~\ref{lemma:bijectivefunction} that states the effect of applying a fixed bijective function to the classical subsystem of a cq-state. This Lemma will be important to prove the security of the quantum oblivious linear evaluation protocol presented in Chapter~\ref{QOLE_chapter}.
...
It will be crucial in the security proof of the quantum oblivious linear evaluation protocol presented in Chapter~\ref{QOLE_chapter} to understand how min-entropy changes when a completely positive (CP) map is applied. It is known that, for a unital CP map $\mathcal{M}$ (i.e. $\mathcal{M}(\mathds{1}) = \mathds{1}$), the conditional min-entropy does not decrease, i.e. $H_{\min}(\mathcal{A}| B) \geq H_{\min}(A| B)$. However, this result is not enough to achieve useful bounds on min-entropy. To achieve a meaningful bound for certain operators $\mathcal{M}$, we can use Theorem~\ref{thm:entaglementSamplingResult} along with some other results (Lemma~\ref{lemma:quantumrelation} and Lemma~\ref{lemma:classicalquantumrelation}). Note that, for readability purposes, the theorem is rewritten to take into account the notation presented in chapter~\ref{QOLE_chapter}.
...
\begin{lemma}[Lemma 17, \cite{this}]
...
\begin{lemma}[Lemma 18, \cite{this}]
