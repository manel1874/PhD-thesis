...
\begin{tcolorbox}[enhanced, 
    ...
    \textit{Transfer phase}:
            ...
            \setcounter{enumi}{4}
                ...
                \item Alice computes the pair of messages $(m_0, m_1)$ as $m_i = f_i(\mathsf{ok}^{\mathsf{S}}_{I_{i}})$.
...
It is important to stress that there is a fundamental difference between QKD and QOT protocols. In QKD, both parties can cooperate to detect an external attack, whereas, in QOT, both parties are distrustful of each other. Moreover, QKD external attacks pressupose that the adversary has access to the quantum channel and is able to play some sort of man-in-the-middle attack. Regarding QOT protocols, both parties can already use their shared quantum channel to attack the other player. 

(FIQUEI AQUI)


In particular, for BBCS-based QOT protocols, Alice must not be able to know which set of indexes is known by Bob (i.e. $\mathsf{e}^\mathsf{B}$) and Bob must have limited knowledge on Alice's key (i.e. $\mathsf{ok}^\mathsf{A} $). However, both parties can use their quantum channel to gain some information (or control) about the set of bases used by the other. 

Two of the most common attacks on quantum systems are faked-state attacks \cite{MH05} (FSA) and trojan-horses attacks \cite{GFKZR06} (THA). The former targets measurement apparatus and the former can target both preparation and measurement apparatus. In a prepare-and-measure setting, FSA can only be used by Alice (sender) while THA can be used by both. 

FSA can be performed with well crafted optical signals that allow Alice to take control over Bob's measurement outcomes. In summary, as described by Jain et al. \cite{JSKEML16}, when both parties' bases coincide, the receiver's detector clicks; when these are incompatible, he gets no detection event ($\bot$). This way, Alice forces Bob to only use the qubit measurements where their bases coincide. So, the indexes corresponding to no detection events will be discarded by both parties whereas the others will be used in the rest of the protocol. This way, the sender has full knowledge of the receiver's bases and can easily distinguish $I_0$ from $I_1$. Note that the sender does not have to attack all measurement turns. He only needs one successful FSA to guess one basis. This happens with high probability in the number of attacks $q$,
$$Pr[\text{Success sender's attack}] = 1 - \bigg(\frac{1}{2}\bigg)^q.$$ This attack is summarized in Figure~\ref{fig:A_FSA}. We denote by $\mathcal{S}_{\textbf{qokd}}(J)$ ($\mathcal{R}_{\textbf{qokd}}(J)$) the sender's (receiver's) quantum hacking procedure that provides him with the receiver's (sender's) bases from index set $J$.

THA is achieved by sending bright pulses into the equipment under attack and scanning through the different reflections to obtain the bases used. Likewise the FSA, the sender only needs one successful attack as summarized in Figure~\ref{fig:A_THA}. However, the receiver's attack is more challenging. Not only he has to successfully guess \textit{all} the sender's bases, he also has to be able to correctly measure the corresponding qubits after leaking the sender's bases. Without the help of quantum memories, this procedure is much more difficult to succeed and allow the receiver to extract the whole key, $k^S$. The receiver's attack based on THA is summarized in Figure~\ref{fig:B_THA}.
...
\fbox{%
    \parbox{\linewidth}{%
    \begin{center}
        \textbf{Sender Faked-states attack}
    \end{center}
    
    S input: $J$ with size $q$.
    
    \begin{enumerate}
         \item S implements some attack $\mathcal{S}_{\textbf{qokd}}(\{J\}) = \{b_{R,j}\}_{j}$ where $b_{R,j}\in\{X, Z\}$ or $b_{R,j}=\bot$. 
         \item If $\exists j$ such that $b_{R,j} \neq \bot$:
         \begin{enumerate}
...
\caption{Sender faked-state attack.}
\label{fig:A_FSA}
\end{center}
\end{figure}
...
\fbox{%
    \parbox{\linewidth}{%
    \begin{center}
        \textbf{Sender Trojan-horse attack}
    \end{center}
    
    S input: $j$.
    
    \begin{enumerate}
         \item S implements some attack $\mathcal{S}_{\textbf{qokd}}(\{j\}) = \{b_{R,j}\}$ where $b_{R,j}\in\{X, Z\}$. 
         \item S compares the received base $b_{R,j}$ with her corresponding base $b_{S,j}$. Denote by $x_j := (b_{R,j} == b_{S,j})$. 
         \item Upon receiving $I_b$ from R:
         \begin{enumerate}
             \item $b=x_j$ if $j\in I_b$;
             \item $b=1-x_j$ if $j\notin I_b$.
         \end{enumerate}
...
\caption{Sender trojan-horse attack.}
\label{fig:A_THA}
\end{center}
\end{figure}
...
\fbox{%
    \parbox{\linewidth}{%
    \begin{center}
        \textbf{Receiver Trojan-horse attack}
    \end{center}
    
    
    \begin{enumerate}
         \item R implements some attack to all qubits sent by S $\mathcal{R}_{\textbf{qokd}} = S$, where $S$ is the set of bases used by S.
         \item R measures the received states with the correct bases, $S$. 
    \end{enumerate} 
...
\caption{Receiver trojan-horse attack.}
\label{fig:B_THA}
\end{center}
\end{figure}
...
%Usually, this comes from the fact that in the theoretical proofs we assume that the devices owned by the parties cannot be corrupted by the adversary. However, in practice this assumption may not happen. Several attacks to QKD implementations have already been reported. Some of this attacks, in particular those presented by Makarov et al. \cite{}, allow the attacker to know the bases being used by the sender's transmitter module. As in the case of QKD systems, this attack is enough to render the QOKD protocol insecure. In fact, a malicious Alice is able to know the quantum basis used by Bob and, consequently, she can easily compute the index set $I_0$ by comparing with her basis choice. Likewise, using the attack set by Makarov, Bob can find Alice basis and measure all his states in the corresponding basis. This would allow him to know Alice's full key, $k^S$.

%Regarding the mitigation measures to prevent attacks coming from insecure implementations, there is a fundamental difference between QKD and QOKD services that must be considered. In the case of QKD systems, Alice and Bob cooperate to prevent external attacks. However, QOKD systems do not have a 

%In order to mitigate these issues 


%Possible mitigation: use QKD mitigation on quantum technology. Possibly develop new protocols (such as the SARG04). Develop a device-independent protocol. Unfortunately, Wehner proved a negative result in the quantum storage model when there are imperfect photon sources. Possibly, it could be used in a commit-and-open model. The logic may be different.

%However, there is still no device-independent protocol for QOKD. There is a negative result in the quantum storage model when there are imperfect photon sources. 


%However, Kaniewski's protocol \cite{KW16} is only proved to be secure against some limited adversary (sequential attacks) and Ribeiro's protocol \cite{RW20} is proved to be not secure against Photon Number Splitting attacks. 

%under the assumption of the absence of quantum memory. Also, this
